...
cv = CountVectorizer()
y_processed = cv.fit_transform(y).toarray()

from sklearn.preprocessing import MinMaxScaler
min_max_scaler = MinMaxScaler(feature_range=(0, 1))
norm_data_y_processed = min_max_scaler.fit_transform(y_processed)

# Binarize the output
from sklearn.preprocessing import label_binarize
y_binarized = label_binarize(norm_data_y_processed, classes=[0, 1, 2, 3, 4])
n_classes = norm_data_y_processed.shape[1]

# Membagi dataset ke dalam Training dan Test set
from sklearn.model_selection import train_test_split
X_train, X_test, y_train, y_test = train_test_split(X, norm_data_y_processed, 
                                                    test_size = 0.2, 
                                                    random_state = 1)
...
#training model deep learning dengan dataset
...
roc_score = roc_auc_score(data_original, data_predictions,multi_class='ovo', average="macro")
print("ROC Score from LSTM = %f%%" % (roc_score*100))
...
fpr["nano"], tpr["nano"], _ = roc_curve(y_test.ravel(), data_predictions.ravel())
...
pre_prediksi = pd.DataFrame(pre_prediksi,columns=['Berat','Berbahaya','Menengah','Normal','Ringan'])
